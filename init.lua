...
do
    ...
    local handlers = {
      python = function()
        ...
        for _, c in ipairs(clients) do
          if c.name == 'ruff' then ruff = c; break end
        end
        if not ruff then
        ...
        ruff:request('workspace/executeCommand', {
          ...
          if err then vim.notify('Error: ' .. vim.inspect(err), vim.log.levels.ERROR) end
          if callback then callback() end
        end)
      end,
      typescript = function()
  ...
  vim.api.nvim_create_user_command('LSPSortImports', function() lsp_sort_imports() end, {
...
do
  ...
  require('flash').setup {}
  vim.keymap.set({ 'n', 'x', 'o' }, '<leader><leader>', function() require('flash').treesitter() end, { desc = 'Flash Treesitter' })
...
do
  ...
  if fzf_ok then
    ...
    vim.keymap.set('n', '<leader>s', function() fzf.files() end, { desc = 'Find files' })
    vim.keymap.set('n', '<leader>dw', function() fzf.live_grep() end, { desc = 'Live grep' })
    vim.keymap.set('n', '<leader>do', function() fzf.oldfiles { cwd_only = true } end, { desc = 'Recent files' })
    vim.keymap.set('n', '<leader>/', function() fzf.blines() end, { desc = 'Search buffer' })
    vim.keymap.set('n', '<leader>da', function() fzf.resume() end, { desc = 'Resume search' })
    vim.keymap.set('n', '<leader>dh', function() fzf.command_history() end, { desc = 'Command history' })
  end
...
do
  ...
  vim.api.nvim_create_autocmd('LspAttach', {
    ...
    callback = function(event)
      ...
      map('<leader>e', function() vim.diagnostic.open_float { border = 'rounded' } end, 'Diagnostic float')
      map('<leader>q', function() require('fzf-lua').diagnostics_document() end, 'Document diagnostics')
      ...
      map('<leader>D', function() require('fzf-lua').lsp_typedefs() end, 'Type Definition')
...
do
  ...
  vim.api.nvim_create_autocmd('FileType', {
    callback = function(args)
      ...
      if vim.treesitter.language.add(lang) then vim.treesitter.start() end
    end,
  })
